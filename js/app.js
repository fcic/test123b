...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (!localStorage.getItem('hasInitializedDefaults')) {
        ...
        // selectedAPIs = ["dyttzy","ruyi","bfzy","tyyszy","xiaomaomi","ffzy","heimuer","zy360","iqiyi","wolong","hwba","jisu","dbzy","mozhua","mdzy","zuid","yinghua","baidu","wujin","wwzy","ikun","lbzy","shayu","tiankong","kuaibo","bajie","lehootv","malimali","duanyou","rrzy","m3u8tv","yidian","qilin","judashen","wuzy9","sugeng","maotaizy","lzi","suoni","z49","uku","guangsu","jinying","kuaiche","xinlang","shandian","jiguang1","jiguang2","zyku1080","kkzy","xiangkan"]
;
...
function resetSearchArea() {
    ...
    try {
        window.history.pushState(
            {},
            `fcicTV - 免费在线视频搜索与观看平台`,
            `/`
        );
        // 更新页面标题
        document.title = `fcicTV - 免费在线视频搜索与观看平台`;
    } catch (e) {
...
async function search() {
    ...
    try {
        ...
        try {
            ...
            // 更新页面标题
            document.title = `搜索: ${query} - fcicTV`;
        } catch (e) {
            console.error('更新浏览器历史失败:', e);
            // 如果更新URL失败，继续执行搜索
        }
...
async function importConfigFromUrl() {
    ...
    document.getElementById('confirmUrlImport').addEventListener('click', async () => {
        ...
        try {
            ...
            if (config.name !== 'fcicTV-Settings') throw '配置文件格式不正确';
...
async function exportConfig() {
    ...
    config['name'] = 'fcicTV-Settings';  // 配置文件名，用于校验
    ...
    // 将配置数据保存为 JSON 文件
    saveStringAsFile(JSON.stringify(config), 'fcicTV-Settings_' + times + '.json');
}
